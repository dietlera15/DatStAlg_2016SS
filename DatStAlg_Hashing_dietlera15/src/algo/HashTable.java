...
public class HashTable {
	...
	public int insertBrent(Object obj) {
		...
		for (int i = 0; i < table.length; i++) {
			if (isFree(h1)) {
				...
				int h2old = hash2(getEntry(h1).hashCode());
				int brentnew = stayPositive(h1 - h2new);
				int brentold = stayPositive(h1 - h2old);
				if (isFree(brentnew) || !isFree(brentold)) {
					h1 = brentnew;
				} else { // !isFree(b1) && isFree(b2)
					setEntry(brentold, getEntry(h1));
					setEntry(h1, null);
				}
			}
		}
	...
	public Object retrieve(int key) {
		// Begin implementation
		int retrieve = searchK(key);
		if (retrieve >= 0) {
			return getEntry(retrieve);
		} else { 
			return null;
		}
		// End implementation
